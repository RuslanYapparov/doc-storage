...
function loadMore(reset = false) {
    ...
    fetch(`/api/v1/docs/owned?${params.toString()}`)
        .then((response) => {
        if (!response.ok) {
...
function search() {
    ...
    if (isFirstSearch) {
        sortParams.from = 0;
        grid.innerHTML = ""; // Очищаем таблицу при первом поиске
        isFirstSearch = false;
    } else {
        sortParams.from = grid.rows.length; // Используем текущую длину таблицы
    }
    ...
    fetch(`/api/v1/docs/search/owned?${params.toString()}`)
        .then((response) => {
        if (!response.ok) {
